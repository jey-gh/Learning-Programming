...
with open('toscrapedotcom.txt', 'w', encoding='utf-8') as file:
    ...
    t_var = True # Helper variable to only print title once.
    ...
    while True:
        ...
        # Scrape homepage title. If statement to only print title once.
        if t_var == True:
            title = soup.find('div', class_='col-sm-8 h1').a.text
            file.write(f'Title: {title} \n\n')
            t_var = False # Set to False, so if statement only runs once.
